...
/*
Helper function for that will assist in calculating the most season wins for each country
*/
const mostSeasonWins = (season, country) => {
  if (['Summer', 'Winter'].includes(season)) {
    return `SELECT year, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}' 
      AND season = '${season}' GROUP BY year ORDER BY COUNT(*) DESC LIMIT 1;`;
  }
}
...
/*
Helper function that allows us to calculate the best year, discipline, sport and event for each country
*/
const countryBestWithCount = (bestThing, country) => {
  if (['year', 'discipline', 'sport', 'event'].includes(bestThing)) {
    return `SELECT ${bestThing}, COUNT(*) AS count FROM GoldMedal 
    WHERE country = '${country}' GROUP BY ${bestThing} ORDER BY COUNT(*) DESC LIMIT 1;`;
  }
}
...
/*
Helper function that allows us to calculate the most wins by gender
*/

const medalsByGender = (gender, country) => {
...
const numberMenMedalists = country => {
  return medalsByGender('Men', country);
};
...
const numberWomenMedalists = country => {
  return medalsByGender('Women', country);
};
...
/*
Helper function that allows to add optional sorting and fields to searches
*/
...
const orderedMedals = (country, field, sortAscending) => {
  ...
  if(field) {
    if(sortAscending) {
      orderedString = `ORDER BY ${field} ASC`;
    } else {
      orderedString = `ORDER BY ${field} DESC`;
    }
  }
  return `SELECT * FROM GoldMedal WHERE country = "${country}" ${orderedString};`;
};
